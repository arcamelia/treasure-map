...
private:
    ...
    // draws a red 7px x 7px square around loc on img
    // loc is at the center of the square
    // if loc is near the edge of the img, only draw the existing portion of the square
    void drawSquare(PNG & img, pair<int,int> loc);
	...
	vector<pair<int,int>> neighbours(const PNG & img, pair<int,int> curr) ;
    ...
    // draws a line from left to right that is 7px long, starting from loc (x, y) on img
    // if (x, y) isn't on img, do nothing
    void drawHorizontalLine(PNG & img, int x, int y);

    void colourRed(PNG & img, int x, int y);

    // returns true if (x, y) coordinates of pixel is within the img
    bool withinBounds(const PNG & img, int x, int y);
